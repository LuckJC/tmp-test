...
// extern int si_mhl_tx_post_initialize(struct mhl_dev_context *dev_context,
// bool bootup); extern void siHdmiTx_VideoSel (int vmode); extern void
// siHdmiTx_AudioSel (int AduioMode); extern void set_platform_bitwidth(int
// bitWidth); extern bool si_mhl_tx_set_path_en_I(struct mhl_dev_context
// *dev_context); extern bool packed_pixel_available(struct mhl_dev_context
// *dev_context); extern void configure_and_send_audio_info(struct
// mhl_dev_context *dev_context, int audio_format);
...
// extern int32_t sii_8348_tx_init(void);    //Should  move to MHL TX Chip user
// layer
/**
...
#define MHL_DBG(fmt, arg...)                                                   \
    do {                                                                       \
        if (hdmi_log_on)                                                       \
            pr_debug("[HDMI_Chip_HAL]%s,%d,", __func__, __LINE__);             \
    } while (0)

#define MHL_FUNC()                                                             \
    do {                                                                       \
        if (hdmi_log_on)                                                       \
            pr_debug("[HDMI_Chip_HAL] %s\n", __func__);                        \
    } while (0)

void hdmi_drv_log_enable(bool enable) { hdmi_log_on = enable; }
...
static int hdmi_drv_video_enable(bool enable) { return 0; }
...
#ifdef CONFIG_MTK_HDMI_3D_SUPPORT
extern void si_mhl_tx_drv_video_3d_update(struct mhl_dev_context *dev_context,
                                          int video_3d);
extern void si_mhl_tx_drv_video_3d(struct mhl_dev_context *dev_context,
                                   int video_3d);
static int hdmi_drv_video_config(enum HDMI_VIDEO_RESOLUTION vformat,
                                 enum HDMI_VIDEO_INPUT_FORMAT vin, int vout)
#else
static int hdmi_drv_video_config(enum HDMI_VIDEO_RESOLUTION vformat,
                                 enum HDMI_VIDEO_INPUT_FORMAT vin,
                                 /*enum HDMI_VIDEO_OUTPUT_FORMAT*/ int vout)
#endif
{
...
power_on_exit:
    ...
    MHL_DBG("status %d, chipid: %x, ret: %d--%d\n", ReadConnectionStatus(),
            chip_device_id, ret, need_reset_usb_switch);
...
void update_av_info_edid(bool audio_video, unsigned int param1,
                         unsigned int param2) {
...
void reset_av_info(void) { pal_resulution = 0; }
...
int hdmi_drv_get_external_device_capablity(void) {
    ...
    // Cap_MAX_channel, Cap_Samplebit, Cap_SampleRate); capablity =
    // Cap_MAX_channel << 3 | Cap_SampleRate << 7 | Cap_Samplebit << 10;

    if (capablity == 0) {
        // capablity = HDMI_CHANNEL_2 << 3 | HDMI_SAMPLERATE_44 << 7 |
        // HDMI_BITWIDTH_16 << 10;
    }
...
void hdmi_unregister_cable_insert_callback(CABLE_INSERT_CALLBACK cb) {
    ...
    if (i == HDMI_MAX_INSERT_CALLBACK) {
        MHL_DBG("Try to unregister callback function 0x%lx which was not "
                "registered\n",
